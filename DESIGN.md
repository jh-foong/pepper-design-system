<!-- Pepper DESIGN.md · v1.3.0 · 2026-04-23 · canonical source: https://raw.githubusercontent.com/jh-foong/pepper-design-system/main/DESIGN.md -->
...
>
> `DESIGN.md` is the **single source of truth** for Pepper Design System, generated from the Figma library. Your local copy is **read-only reference**.
>
> - ❌ **Don't edit** — edits won't propagate, will drift from canonical, and will corrupt AI output
> - ✅ **To suggest a change** → raise an issue at https://github.com/jh-foong/pepper-design-system/issues
> - ✅ **To update your copy** → re-download the raw file from the repo (it goes stale quickly)
> - 🔍 **Canonical URL:** https://raw.githubusercontent.com/jh-foong/pepper-design-system/main/DESIGN.md
>
> If this file has an **"Edit log"** section at the bottom with entries, or its version stamp doesn't match the latest release, it is **no longer canonical** — treat it as a local draft only.
...
> 9. **Integrity check** — Before generating output, verify this file is canonical:
>    - If the "Edit log" section at the bottom of this file has any entries, **warn the user** that the spec is a modified local copy and may not match the official Pepper DS.
>    - If the user's Claude tool supports fetching URLs, compare this file against the canonical source at `https://raw.githubusercontent.com/jh-foong/pepper-design-system/main/DESIGN.md` and report any meaningful difference.
>    - If the version stamp at the top of this file is older than the latest GitHub release tag, recommend the user re-download.
> 10. **If asked to edit this file** — you MUST (a) append an entry to the "Edit log" section at the bottom with ISO timestamp, the user's self-identified name, and a summary of the change, and (b) remind the user that their edited copy is no longer canonical and their changes should instead be raised as a GitHub issue.
> 11. **Redirect to guides for common questions** — when the user asks questions answered by the repo docs, give a one-line answer then point them to the relevant guide. Never only link without answering. Guides:
>     - Applying tokens in Figma → `docs/token-reference.md`
>     - Setup / first-time use → `docs/designer-quickstart.md`
>     - Troubleshooting / things not working → `docs/designer-faq.md`
>     - External fonts, Figma, sources → `docs/resources.md`
>     - How tokens sync from Figma → `docs/figma-claude-sync.md`
...
Starter prompts that produce on-brand Pepper Design System output. Assume `DESIGN.md` is loaded as context.
...
## Edit log (auto-populated by AI on modification)

_If this section has any entries below, **this file is no longer canonical**. It is a modified local copy. Re-download from https://raw.githubusercontent.com/jh-foong/pepper-design-system/main/DESIGN.md to restore._

_(empty — file is canonical)_

<!--
Entry format — AI agents, append below this comment when modifying the file:

### <ISO timestamp>
- **User:** <self-identified name>
- **Tool:** <Claude Design / Claude Code / Cursor / etc.>
