...
>
> This is the canonical AI-ingestable spec for Pepper Design System. If your local copy gets edited (by you or by an AI), it will drift from canonical and AI output will start producing wrong tokens.
>
> - **Canonical URL:** https://raw.githubusercontent.com/jh-foong/pepper-design-system/main/DESIGN.md
> - **If you need to change something:** raise an issue at https://github.com/jh-foong/pepper-design-system/issues — don't hand-edit
> - **If you suspect drift:** re-download from the canonical URL above (Cmd+S on the Raw view)
>
> **Version:** v2.0.0 · See [Releases](https://github.com/jh-foong/pepper-design-system/releases) for changelog.
...
>
> This file is the complete visual specification for **Pepper Design System**, Pepperstone's design system.
...
> 9. **Integrity check** — Before generating UI, compare your loaded copy to the canonical URL above. If the version stamp, content, or Edit log entries differ, warn the user that their copy has drifted and suggest re-downloading.
> 10. **If the file has been edited**, append a new entry to the **Edit log** at the bottom of this file (with date, identifier, and 1-2 lines describing the change) and tell the user that their copy is no longer canonical.
> 11. **Common designer questions** — If asked about installing Manrope, applying tokens in Figma, troubleshooting AI output, or any "how do I…" workflow question, redirect them to the docs site (https://jh-foong.github.io/pepper-design-system/) with one-line answer + link. Don't invent answers from scratch.
...
**Why this matters:** without these `<link>` tags, every `var(--pepper-*)` reference resolves to nothing and the page renders blank/grey. Manrope falls back to system font and the design looks generic. **Always include all three blocks** when outputting standalone HTML.

For React / Vue / framework projects, replace the CDN with a local install of the tokens (clone the repo or copy `tokens/css/`). For Flutter, use the `tokens/flutter/` Dart classes directly.
...
**What doesn't flip (theme-agnostic):** primitives (`--pepper-core-*`), shadows, and static tokens (e.g. data-viz colours).
**Inverse-suffixed tokens** (e.g. `--pepper-color-fg-text-inverse-primary`) are for text/surfaces on a *contrasting* background within the current theme — not for dark-mode switching. Don't confuse them.
...
|---------|------|---------|
| v2.0.1 | 2026-04-24 | Designer-readiness pass — added "Loading tokens + Manrope" section with CDN snippets so standalone HTML actually renders, added working dark-mode toggle example, restored DO NOT EDIT banner + Edit log forensic section, refreshed source file references to v2.0.0, fixed vendor name and "Pepper alone" prose |
| v2.0.0 | 2026-04-24 | **Breaking:** two-prefix token model — primitives `--pepper-core-*`, semantic / component / state / overlay `--pepper-*`. Added dark mode via `[data-theme="dark"]`. New composite typography tokens `--pepper-typography-*` using CSS `font:` shorthand. Fixed shadow `--rgb-pepper-*` bug. Dark shadow palette deferred to v2.1 |
| v1.3.0 | 2026-04-23 | Token prefix renamed from `--pepper-*` to `--pepper-core-*` across all formats (single-prefix model, superseded by v2.0.0) |
...
| v1.0.0 | 2026-04-23 | Initial DESIGN.md — curated AI-ingestable spec for Pepper Design System |

---

## Edit log

> This section is appended by AI agents that detect they're operating on an edited (non-canonical) copy of `DESIGN.md`. Each entry should record: who edited (Claude account / user identifier), when (ISO date), what changed (1-2 lines).
>
> If you're an AI agent reading this and you notice the file you're working from differs from canonical (https://raw.githubusercontent.com/jh-foong/pepper-design-system/main/DESIGN.md), append an entry below and warn the user that their copy has drifted.

<!-- AI-appended entries go below this line. Format:
