...
- **Grid**: 4px base unit
- **Theme**: Light mode by default. Dark mode is enabled by setting `data-theme="dark"` on any ancestor element — semantic colour, component, overlay-highlight, state-overlay, and gradient-stop tokens automatically flip. Inverse-suffixed tokens (e.g. `--pepper-color-fg-text-inverse-primary`) are for text/surfaces placed on a *contrasting* background within the current theme, not for dark-mode switching.
...
All semantic colours are referenced via CSS custom properties with prefixes like `--pepper-color-fg-*` (foreground: text, stroke, icon), `--pepper-color-bg-*` (backgrounds / surfaces), `--pepper-color-component-*` (component-specific), and `--pepper-color-static-*` (theme-agnostic, e.g. data-viz).
